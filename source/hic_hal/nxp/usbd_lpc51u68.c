...
void USBD_Init(void)
{
    ...
    SYSCON->USBCLKDIV = SYSCON_USBCLKDIV_DIV(0);
    #endif
...
void USBD_Reset(void)
{
    ...
    *ptr = BUF_ADDR(EPBufInfo[0].buf_ptr + 64);/* SETUP */
    ...
    USB0->INTEN = ((1UL << 30) |            /* SOF intr enable */
...
{
    SYSCON->USBCLKCTRL = 1;
    USB0->DEVCMDSTAT &= ~(1UL << 17);  /*clear device suspend status */

    while (USB0->DEVCMDSTAT & (1UL << 17));
...
{
    if (cfg == __TRUE) {
        USB0->DEVCMDSTAT &= ~(1UL << 9); /*PPL_ON=0, in suspend clk is stoped */
    } else {
        USB0->DEVCMDSTAT |= (1UL << 9);  /*PPL_ON=1, in suspend clk isnt stoped */
        SYSCON->USBCLKCTRL = 0;
    }
}
...
void USBD_SetStallEP(U32 EPNum)
{
    ...
    if (EPNum & 0x7F) {
        ...
        if (EPNum & 0x80) {
            ...
            USB0->EPSKIP |= (1 << EP_OUT_IDX(EPNum));

            while (USB0->EPSKIP & (1 << EP_OUT_IDX(EPNum)));
        }
    }
...
U32 USBD_ReadEP(U32 EPNum, U8 *pData, U32 size)
{
    ...
    /* Setup packet */
    if ((EPNum == 0) && !ctrl_out_next && (USB0->DEVCMDSTAT & (1UL << 8))) {
...
void USBD_Handler(void)
{
    ...
    if (sts & 0x3FF) {
        ...
        for (i = 0; i < endpoint_count; i++) {
            ...
            if (sts & (1UL << num)) {
                /* Setup */
                if ((num == 0) && !ctrl_out_next && (USB0->DEVCMDSTAT & (1UL << 8))) {
#ifdef __RTX
