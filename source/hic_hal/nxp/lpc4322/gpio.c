...
void gpio_init(void)
{
    ...
    scu_pinmux(3, 1, GPIO_NOPULL, FUNC4);   /* POWER_EN: GPIO5[8]  */
    ...
    /* Configure: POWER_ON as output (turned off) */
    LPC_GPIO_PORT->CLR[POWER_EN_PORT]  = (1 << POWER_EN_BIT);
    LPC_GPIO_PORT->DIR[POWER_EN_PORT] |= (1 << POWER_EN_BIT);
...
{
    if (powerEnabled) {
        LPC_GPIO_PORT->SET[POWER_EN_PORT] = (1 << POWER_EN_BIT);
    } else {
        LPC_GPIO_PORT->CLR[POWER_EN_PORT] = (1 << POWER_EN_BIT);
    }
}
